...
/**
 * Created by wujintao on 17-8-28.
 */
object zyyRDDKmeansplusTest {
  def main (args: Array[String]){
    ...
    val t=zyyKmeansN(rddData,20,100,5)
    val da=t._1.collect()
    for(temp<-da){
      print(temp._1+",")
      printArr(temp._2)
...
//  }
  ...
  def plusPreInitK(dataS:org.apache.spark.rdd.RDD[(Long,Array[Double],Int)],k:Int):Array[(Long,Array[Double],Int)]={
    ...
    val sample:Array[(Long,Array[Double],Int)]=dataS.takeSample(true,1,n)

    re(0)=sample(0)

    for(i<-1 until k){
      val tempDataS=dataS.map(x=>(x._1,x._2,x._3,computData_Cent_min(x,re,i))).cache()
      ...
      if(tempCent.count()>0){
        ...
        val n=ran.nextInt(10000)
        val c:Array[(Long,Array[Double],Int)]=dataS.takeSample(true,1,n)
        re(i)=c(0)
      }
  ...
  //计算数据到中心的最小距离
  def computData_Cent_min(item:(Long,Array[Double],Int),currentCents:Array[(Long,Array[Double],Int)],currentI:Int):Double={
